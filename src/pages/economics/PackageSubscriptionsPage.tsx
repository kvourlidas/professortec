...
import {
  ...
  CalendarDays, Repeat, CheckCircle2, XCircle, ChevronDown,
} from 'lucide-react';
...
// Display ISO date (yyyy-mm-dd) as dd/mm/yyyy, passthrough if already display format
...
export default function PackageSubscriptionsPage() {
  ...
  // track which yearly cards have their date section expanded
  ...
  const dateInputCls = isDark
    ? 'w-full rounded-lg border border-slate-700/60 bg-slate-900/60 px-3 py-2 text-xs text-slate-100 placeholder-slate-600 outline-none transition focus:border-[color:var(--color-accent)]/60'
    : 'w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 placeholder-slate-400 outline-none transition focus:border-[color:var(--color-accent)]/60';
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase
      .from('packages')
      .select('id,school_id,name,price,currency,is_active,sort_order,package_type,hours,starts_on,ends_on')
  ...
  const openAdd = () => {
    setNewName(''); setNewType('monthly'); setNewHours('');
    setNewPrice('0.00'); setNewActive(true);
    ...
    <div className="space-y-6 px-1">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button type="button" onClick={openAdd}
          className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-semibold transition hover:brightness-110 active:scale-[0.97] ${isDark ? 'text-black' : 'text-white'}`}
            ...
            <div className="space-y-2.5">
                ...
                return (
                  <div key={r.id} className={pkgCardCls}>
                    ...
                    <div className="absolute left-0 top-3 bottom-3 w-0.5 rounded-full" style={{
                      background: r.package_type === 'hourly' ? 'rgba(56,189,248,0.5)'
                        : r.package_type === 'monthly' ? 'rgba(167,139,250,0.5)'
                        : 'rgba(251,191,36,0.5)',
                    }} />

                    {/* Main row */}
                    <div className="flex flex-col gap-3 p-4 pl-5 lg:flex-row lg:items-center lg:gap-4">
                      {/* Type badge + Name */}
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <select value={r.package_type}
                          onChange={e => { const t = e.target.value as PackageType; updateRow(r.id, { package_type: t, hours: t === 'hourly' ? r.hours : '' }); }}
                          className={selectCls}>
                        ...
                        {/* Date range toggle — only for yearly */}
                    ...
                    {/* ── Expandable date range section (yearly only) ── */}
                    {r.package_type === 'yearly' && dateExpanded && (() => {
                      const onStartsOnChange = (v: string) => updateRow(r.id, { starts_on: v });
                      const onEndsOnChange   = (v: string) => updateRow(r.id, { ends_on: v });
                      return (
                        <div className={`border-t px-5 py-3 ${isDark ? 'border-slate-800/60 bg-slate-900/20' : 'border-slate-100 bg-slate-50/60'}`}>
                          <p className={`mb-2.5 text-[10px] font-semibold uppercase tracking-widest ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                            Διάστημα ετήσιας συνδρομής
                          </p>
                          <div className="flex flex-wrap items-center gap-3">
                            <div className="flex items-center gap-2">
                              <span className={`text-[10px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Έναρξη</span>
                              <div className="w-44">
                                <AppDatePicker value={r.starts_on} onChange={onStartsOnChange} />
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div className="w-44">
                                <AppDatePicker value={r.ends_on} onChange={onEndsOnChange} />
                              </div>
                            </div>
                            <p className={`text-[10px] ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>
                              Αυτό το διάστημα θα χρησιμοποιείται αυτόματα κατά την ανάθεση σε μαθητή.
                            </p>
                          </div>
                        </div>
                      );
              ...
              <div className={`flex items-center justify-end gap-2 pt-3 border-t ${isDark ? 'border-slate-800/60' : 'border-slate-200'}`}>
                ...
                <button type="button" onClick={saveAll} disabled={!isDirty || saving}
                  className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-semibold transition hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed ${isDark ? 'text-black' : 'text-white'}`}
                  style={{ background: isDark ? 'var(--color-accent)' : '#2563eb' }}>
          ...
          <div className={modalCardCls} style={isDark ? { background: 'var(--color-sidebar)' } : {}}>
            ...
            <div className={`flex items-center justify-between px-6 pt-5 pb-4 ${!isDark ? 'border-b border-slate-100' : ''}`}>
              ...
              <button type="button" onClick={() => { setAddOpen(false); setAddError(null); }}
                className={`flex h-7 w-7 items-center justify-center rounded-lg border transition text-xs ${isDark ? 'border-slate-700/60 bg-slate-800/50 text-slate-400 hover:text-slate-200' : 'border-slate-200 bg-slate-100 text-slate-500 hover:text-slate-700'}`}>✕</button>
            </div>
            ...
            <div className="space-y-4 px-6 pb-2">
              <div>
                ...
                <input value={newName} onChange={e => setNewName(e.target.value)} className={inputCls} placeholder="π.χ. Ετήσια συνδρομή" />
              </div>
              ...
              {/* Date range — only for yearly */}
              {newType === 'yearly' && (
                <div>
                  <label className={labelCls}>Διάστημα συνδρομής</label>
                  <div className="flex items-center gap-2">
                    <div className="flex-1"><AppDatePicker value={newStartsOn} onChange={setNewStartsOn} /></div>
                    <span className={isDark ? 'text-slate-600' : 'text-slate-400'}>—</span>
                    <div className="flex-1"><AppDatePicker value={newEndsOn} onChange={setNewEndsOn} /></div>
                  </div>
                  <p className={`mt-1 text-[11px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Αυτόματα προσυμπληρώνεται κατά την ανάθεση σε μαθητή.</p>
                </div>
              )}
            ...
            <div className={`flex justify-end gap-2.5 px-6 py-4 ${isDark ? 'mt-2 border-t border-slate-800/70 bg-slate-900/20' : 'border-t border-slate-100 bg-slate-50'}`}>
              ...
              <button type="button" onClick={addPackage} disabled={saving}
                className={`flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-semibold transition hover:brightness-110 active:scale-[0.97] disabled:opacity-60 ${isDark ? 'text-black' : 'text-white'}`}
                style={{ background: isDark ? 'var(--color-accent)' : '#2563eb' }}>
                {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Plus className="h-3.5 w-3.5" />}Προσθήκη
              </button>
            </div>
          </div>
        </div>

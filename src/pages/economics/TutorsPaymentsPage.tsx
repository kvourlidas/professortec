...
const FieldLabel = ({children}: {children: React.ReactNode}) => (
  <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500">{children}</div>
);
  ...
  return (
    <div className="space-y-6 px-1">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-base font-semibold tracking-tight text-slate-50">Πληρωμές Καθηγητών</h1>
          <p className="mt-0.5 text-xs text-slate-400">Βασική αμοιβή, μπόνους και ιστορικό πληρωμών ανά καθηγητή.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        <div className="lg:col-span-4">
          <div className="overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-950/40 shadow-xl backdrop-blur-md ring-1 ring-inset ring-white/[0.04]">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-slate-800/60 bg-slate-900/30 px-4 py-3">
              <span className="text-xs font-semibold text-slate-200">Καθηγητές</span>
              <span className="rounded-full border border-slate-700/60 bg-slate-900/40 px-2 py-0.5 text-[10px] font-semibold text-slate-400">{tutors.length}</span>
            </div>
            ...
            <div className="border-b border-slate-800/60 px-3 py-3">
              <div className="relative">
                ...
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Αναζήτηση καθηγητή..."
                  className="h-8 w-full rounded-lg border border-slate-700/60 bg-slate-900/60 pl-8 pr-3 text-xs text-slate-100 placeholder-slate-500 outline-none transition focus:border-[color:var(--color-accent)]"/>
              </div>
            </div>
            ...
            <div className="max-h-[480px] overflow-y-auto divide-y divide-slate-800/40">
              {filteredTutors.map(t => {
                ...
                return (
                  <div key={t.id} className={`flex items-center justify-between gap-3 px-4 py-3 transition-colors ${active ? 'bg-white/[0.07]' : 'hover:bg-white/[0.03]'}`}>
                    <div className="min-w-0">
                      <div className={`text-xs font-semibold ${active ? 'text-slate-50' : 'text-slate-200'}`}>{t.full_name??'—'}</div>
                    ...
                    <button type="button" onClick={() => setSelectedTutorId(t.id)} title="Άνοιγμα"
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border transition ${active ? 'border-[color:var(--color-accent)]/40 text-[color:var(--color-accent)]' : 'border-slate-700/60 bg-slate-900/30 text-slate-400 hover:border-slate-600 hover:text-slate-200'}`}
              ...
              {filteredTutors.length === 0 && <div className="px-4 py-6 text-center text-xs text-slate-500">Δεν βρέθηκαν καθηγητές.</div>}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          {!selectedTutor ? (
            <div className="flex items-center justify-center rounded-2xl border border-slate-700/50 bg-slate-950/40 p-12 text-xs text-slate-500 backdrop-blur-md">
            ...
            <div className="space-y-4">
              ...
              {/* Base pay card */}
              <div className="overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-950/40 shadow-xl backdrop-blur-md ring-1 ring-inset ring-white/[0.04]">
                <div className="flex items-center justify-between border-b border-slate-800/60 bg-slate-900/30 px-5 py-3">
                  <div>
                    <span className="text-xs font-semibold" style={{ color: 'var(--color-accent)' }}>{selectedTutor.full_name??'—'}</span>
                    <span className="ml-2 text-[11px] text-slate-500">· Βασική αμοιβή</span>
                  </div>
                  <button type="button" onClick={saveBasePay} disabled={busy} title="Αποθήκευση"
                    className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-700/60 bg-slate-800/50 text-slate-400 transition hover:text-slate-200 disabled:opacity-50">
                ...
                <div className="p-5">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <div><FieldLabel>Νόμισμα</FieldLabel><div className="flex h-9 items-center rounded-lg border border-slate-700/60 bg-slate-900/40 px-3 text-xs font-semibold text-slate-300"><Euro className="mr-1.5 h-3 w-3 text-slate-500"/>{CURRENCY_SYMBOL}</div></div>
                  </div>
                  <div className="mt-4 flex flex-col gap-2 md:flex-row md:items-end">
                    ...
                    <button type="button" onClick={recordPaymentToday} disabled={busy}
                      className="inline-flex h-9 shrink-0 items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-4 text-xs font-semibold text-emerald-300 transition hover:bg-emerald-500/20 disabled:opacity-50">
              ...
              {/* Bonus card */}
              <div className="overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-950/40 shadow-xl backdrop-blur-md ring-1 ring-inset ring-white/[0.04]">
                <div className="border-b border-slate-800/60 bg-slate-900/30 px-5 py-3">
                  <span className="text-xs font-semibold text-slate-200">Μπόνους</span>
                </div>
                <div className="p-5">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-12">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <button type="button" onClick={addBonus} disabled={busy||(Number(bonusValue)||0)<=0}
                        className="inline-flex h-9 w-full items-center justify-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 text-xs font-semibold text-emerald-300 transition hover:bg-emerald-500/20 disabled:opacity-40">
              ...
              {/* History card */}
              <div ref={historyRef} className="overflow-hidden rounded-2xl border border-slate-700/50 bg-slate-950/40 shadow-xl backdrop-blur-md ring-1 ring-inset ring-white/[0.04]">
                <div className="flex items-center justify-between border-b border-slate-800/60 bg-slate-900/30 px-5 py-3">
                  <div className="flex items-center gap-2">
                    <History className="h-3.5 w-3.5 text-slate-400"/>
                    <span className="text-xs font-semibold text-slate-200">Ιστορικό Πληρωμών</span>
                  </div>
                  {payments.length > PAGE_SIZE && (
                    <div className="flex items-center gap-1">
                      <button type="button" onClick={()=>setHistoryPage(p=>Math.max(0,p-1))} disabled={historyPage===0}
                        className="flex h-6 w-6 items-center justify-center rounded-lg border border-slate-700/60 bg-slate-900/40 text-slate-400 transition hover:bg-slate-800/50 disabled:opacity-30">
                        <ChevronLeft className="h-3 w-3"/>
                      </button>
                      <span className="min-w-[48px] text-center text-[11px] text-slate-400"><span className="font-semibold text-slate-200">{historyPage+1}</span> / {totalPages}</span>
                ...
                {payments.length === 0 ? (
                  <div className="flex items-center justify-center py-10 text-xs text-slate-500">Δεν υπάρχει ιστορικό.</div>
                ) : (
                  <>
                    ...
                    <div className="divide-y divide-slate-800/40">
                      {pagePayments.map(p => (
                        <div key={p.id} className="grid grid-cols-12 items-center px-4 py-2.5 text-xs transition-colors hover:bg-white/[0.02]">
                          <div className="col-span-2 tabular-nums text-slate-400">{isoDateFromTs(p.paid_on??p.created_at)}</div>
                          <div className="col-span-2 font-medium tabular-nums text-slate-200">{money(p.net_total)} {CURRENCY_SYMBOL}</div>
                          <div className="col-span-2 tabular-nums text-slate-300">{money(p.gross_total)} {CURRENCY_SYMBOL}</div>
                          <div className="col-span-2 tabular-nums text-slate-400">{money(p.bonus_total)} {CURRENCY_SYMBOL}</div>
                          <div className="col-span-2">
                            <span className="inline-flex rounded-full border border-emerald-700/50 bg-emerald-950/30 px-2 py-0.5 text-[10px] font-medium text-emerald-300">Πληρώθηκε</span>
                          </div>
                          <div className="col-span-2 flex justify-end gap-1.5">
                            <button type="button" onClick={()=>openEditPayment(p)} disabled={busy}
                              className="flex h-7 w-7 items-center justify-center rounded-lg border border-blue-700/50 bg-blue-950/30 text-blue-300 transition hover:bg-blue-950/50 disabled:opacity-50" title="Επεξεργασία">
                              <Pencil className="h-3 w-3"/>
                            </button>
                            <button type="button" onClick={()=>askDeletePayment(p)} disabled={busy}
                              className="flex h-7 w-7 items-center justify-center rounded-lg border border-rose-800/50 bg-rose-950/30 text-rose-300 transition hover:bg-rose-950/50 disabled:opacity-50" title="Διαγραφή">
      ...
      <ConfirmActionModal
        ...
        message={<div className="text-slate-200">Σίγουρα θέλετε να διαγράψετε αυτή την εγγραφή πληρωμής <span className="font-semibold text-slate-50">({deletingPayment ? isoDateFromTs(deletingPayment.paid_on??deletingPayment.created_at) : '—'})</span>; Η ενέργεια αυτή δεν μπορεί να ανακληθεί.</div>}
      ...
      {editOpen && editingPayment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-700/60 shadow-2xl" style={{ background: 'var(--color-sidebar)' }}>
            ...
            <div className="flex items-center justify-between px-6 pt-5 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] text-slate-400">Ημερομηνία: {isoDateFromTs(editingPayment.paid_on??editingPayment.created_at)}</p>
                </div>
              </div>
              <button type="button" onClick={closeEditPayment} disabled={busy} className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-700/60 bg-slate-800/50 text-slate-400 transition hover:text-slate-200 disabled:opacity-50">
            ...
            <div className="mt-5 flex justify-end gap-2.5 border-t border-slate-800/70 bg-slate-900/20 px-6 py-4">
              <button type="button" onClick={closeEditPayment} disabled={busy} className="rounded-lg border border-slate-600/60 bg-slate-800/50 px-4 py-1.5 text-xs font-medium text-slate-200 transition hover:bg-slate-700/60 disabled:opacity-50">Ακύρωση</button>
              <button type="button" onClick={saveEditedPayment} disabled={busy} className="inline-flex items-center gap-1.5 rounded-lg px-4 py-1.5 text-xs font-semibold text-black transition hover:brightness-110 disabled:opacity-60" style={{ backgroundColor: 'var(--color-accent)' }}>

...
import type { AddSlotForm, StudentRow, SubjectRow } from './types';

interface PrivateProgramAddSlotModalProps {
  open: boolean;
  form: AddSlotForm;
  saving: boolean;
  error: string | null;
  ...
  onClose: () => void;
  onSubmit: () => void;
  onFieldChange: (field: 'subjectId') => (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void;
...
export default function PrivateProgramAddSlotModal({
...
}: PrivateProgramAddSlotModalProps) {
  if (!open) return null;
      ...
      <div className={modalCardCls} style={{ background: 'var(--color-sidebar)' }}>
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--ch-divider)' }}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wide" style={{ color: 'var(--ch-text)' }}>Προσθήκη μαθήματος</h2>
          </div>

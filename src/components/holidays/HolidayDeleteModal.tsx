...
type HolidayDeleteModalProps = {
  deleteGroup: HolidayGroup | null;
  deleting: boolean;
  onCancel: () => void;
...
export default function HolidayDeleteModal({ deleteGroup, deleting, onCancel, onConfirm }: HolidayDeleteModalProps) {
  ...
  const rangeLabel = deleteGroup.endDate && deleteGroup.endDate !== deleteGroup.startDate
        ...
        <div className="p-6">
          ...
          <p className={`text-xs leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
            ...
            {deleteGroup.name
              ...
              : 'την αργία '}
            για{' '}
            <span className={`font-semibold ${isDark ? 'text-slate-100' : 'text-slate-800'}`}>{rangeLabel}</span>;{' '}

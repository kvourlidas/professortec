...
import {
  ...
  AlertCircle, AlertTriangle, CheckCircle2, XCircle,
} from 'lucide-react';
...
import { downloadStudentImportTemplate } from './importStudentsShared.ts';
import { parseStudentImportFile, type ParsedStudentRow } from './parseStudentImportFile.ts';
...
export default function StudentImportModal({ schoolId, levels, onImported, onClose }: Props) {
  ...
  const handleFileChange = async (file: File | null) => {
    ...
    try {
      const parsed = await parseStudentImportFile(file, levels);
      if (parsed.length === 0) {
        setParseError('Δεν βρέθηκαν γραμμές μαθητών στο αρχείο.');
        return;
      }
      setRows(parsed);
      setStep('preview');
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < validRows.length; i++) {
      ...
      const payload = {
        ...
        father_date_of_birth: r.father_date_of_birth,
        father_phone: r.father_phone,
        father_email: r.father_email,
        father_afm: r.father_afm,
        mother_name: r.mother_name,
        mother_date_of_birth: r.mother_date_of_birth,
        mother_phone: r.mother_phone,
        mother_email: r.mother_email,
        mother_afm: r.mother_afm,
      };

      try {
        const { data, error } = await supabase.functions.invoke('student-create', { body: payload });
        if (error || !data?.item) {
          rowResults.push({ rowNumber: r.rowNumber, full_name: r.full_name, status: 'failed', message: 'Αποτυχία δημιουργίας' });
        } else {
          const item = data.item as StudentRow;
          created.push(item);
          rowResults.push({ rowNumber: r.rowNumber, full_name: r.full_name, status: 'success' });

          if (r.password && (r.email || r.phone)) {
            const { error: userErr } = await supabase.functions.invoke('create-student-user', {
              body: { school_id: schoolId, student_id: item.id, email: r.email, phone: r.phone, password: r.password },
            });
            if (userErr) console.error('create-student-user error for row', r.rowNumber, userErr);
          }
        }
      } catch (err) {
      ...
      <div className={`relative w-full max-w-2xl overflow-hidden rounded-2xl border shadow-2xl ${modalBg}`}>
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--ch-divider)' }}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wide" style={{ color: 'var(--ch-text)' }}>Εισαγωγή μαθητών από Excel</h2>
          </div>
          ...
          {step === 'select' && (
            <div className="space-y-5">
              <p className={`text-xs leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                Κατεβάστε το πρότυπο αρχείο Excel, συμπληρώστε τα στοιχεία των μαθητών και ανεβάστε το εδώ για μαζική εισαγωγή.
              </p>
              ...
              <button type="button" onClick={handleDownloadTemplate} disabled={templateBusy}
                ...
                <div>
                  ...
                  <p className={`text-[11px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>Μία γραμμή ανά μαθητή, κάτω από τις επικεφαλίδες</p>
                </div>
              </button>
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${isDark ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' : 'border-emerald-200 bg-emerald-50 text-emerald-600'}`}>
                  <CheckCircle2 className="h-3 w-3" />{validRows.length} έτοιμες για εισαγωγή
                </span>
                      ...
                      <tr key={r.rowNumber}>
                        ...
                        <td className="px-3 py-2">
                          {r.errors.length > 0 ? (
                            ...
                            <span className="flex items-start gap-1.5 text-amber-500"><AlertTriangle className="mt-0.5 h-3 w-3 shrink-0" />{r.warnings.join('· ')}</span>
                          ) : (
          ...
          {step === 'preview' && (
            <>
              ...
              <button type="button" onClick={handleImport} disabled={validRows.length === 0}
                ...
                Εισαγωγή {validRows.length} μαθητών
              </button>
            </>
          )}
          {step === 'importing' && (

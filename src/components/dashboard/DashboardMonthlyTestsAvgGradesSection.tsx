...
function DropdownShell({ label, open, onToggle, children, widthClass }: {
  label: string; open: boolean; onToggle: () => void; children: React.ReactNode; widthClass?: string;
}) {
  return (
    <div className="relative">
      <button type="button" onClick={onToggle}
        className={`inline-flex items-center justify-between gap-2 rounded-lg border border-slate-700/60 bg-slate-800/50 px-2.5 py-1.5 text-[11px] text-slate-300 transition hover:border-slate-600 hover:bg-slate-700/60 ${widthClass ?? 'w-[150px]'}`}
        aria-haspopup="dialog" aria-expanded={open}>
        <span className="truncate">{label}</span>
        <ChevronDown className="h-3.5 w-3.5 shrink-0 text-slate-500" />
      </button>
      {open && (
        <div className="absolute left-0 z-50 mt-1.5 w-full overflow-hidden rounded-xl border border-slate-700/60 bg-slate-900/95 shadow-xl backdrop-blur-xl" role="dialog">
          {children}
        </div>
      )}
    </div>
...
function BarTooltip({ active, payload, label }: { active?: boolean; payload?: any[]; label?: string }) {
  ...
  if (!row) return null;
  return (
    <div className="rounded-xl border border-slate-700/60 bg-slate-900/95 px-3 py-2 text-[11px] text-slate-100 shadow-xl backdrop-blur-xl">
      <p className="font-semibold">{label}</p>
      <p className="mt-1 text-slate-300">Μ.Ο.: <span className="font-semibold text-slate-100">{row.avg}</span> <span className="text-slate-500">({row.count} βαθμοί)</span></p>
    </div>
  );
}

const listItemCls = (active: boolean) => `w-full flex items-center justify-between gap-2 px-3 py-2 text-[11px] rounded-lg transition cursor-pointer ${active ? 'bg-white/10 text-slate-100' : 'text-slate-300 hover:bg-white/[0.06] hover:text-slate-100'}`;
...
export default function DashboardMonthlyTestsAvgGradesSection({ schoolId }: Props) {
  ...
  useOutsideClose([monthWrapRef, yearWrapRef, subjectsWrapRef], () => { setOpenMonth(false); setOpenYear(false); setOpenSubjects(false); }, openMonth || openYear || openSubjects);
  ...
  const yearOptions = useMemo(() => { const base = new Date().getFullYear(); const arr: number[] = []; for (let y = base - 10; y <= base + 10; y++) arr.push(y); return arr; }, []);
  ...
  const yDomain = useMemo(() => { const maxVal = Math.max(0, ...rows.map((r) => r.avg)); if (maxVal <= 20) return [0, 20] as [number, number]; return [0, Math.ceil(maxVal / 10) * 10] as [number, number]; }, [rows]);
      ...
      <div className="px-5 py-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-50">Τεστ & Μέσος Όρος</p>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <div ref={subjectsWrapRef}>
              <DropdownShell label={subjectsLabel} open={openSubjects} onToggle={() => { setOpenMonth(false); setOpenYear(false); setOpenSubjects((v) => !v); }} widthClass="w-[180px]">
                <div className="max-h-64 overflow-y-auto p-1.5">
                  <button type="button" onClick={() => { setSelectedSubjectIds([]); setOpenSubjects(false); }} className={listItemCls(selectedSubjectIds.length === 0)}>
                    ...
                    {selectedSubjectIds.length === 0 && <Check className="h-3 w-3 text-slate-300" />}
                  </button>
                  <div className="my-1 h-px bg-slate-800/80" />
                  {subjects.length === 0 ? (
                    <p className="px-3 py-2 text-[11px] text-slate-500">Δεν υπάρχουν μαθήματα.</p>
                  ) : subjects.map((s) => {
            ...
            <div ref={monthWrapRef}>
              <DropdownShell label={monthLabel} open={openMonth} onToggle={() => { setOpenSubjects(false); setOpenYear(false); setOpenMonth((v) => !v); }} widthClass="w-[140px]">
                <div className="max-h-64 overflow-y-auto p-1.5">
                  {MONTHS.map((m) => (
            ...
            <div ref={yearWrapRef}>
              <DropdownShell label={String(year)} open={openYear} onToggle={() => { setOpenSubjects(false); setOpenMonth(false); setOpenYear((v) => !v); }} widthClass="w-[80px]">
                <div className="max-h-64 overflow-y-auto p-1.5">
                  {yearOptions.map((y) => (
        ...
        <div className="mt-4 h-48">
          {loading ? (
            ...
            <div className="flex items-start gap-3 rounded-xl border border-red-500/30 bg-red-950/40 px-4 py-3 text-xs text-red-200 backdrop-blur">
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
              ...
              <p className="text-xs text-slate-500">Δεν υπάρχουν βαθμολογίες για αυτόν τον μήνα.</p>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={rows} margin={{ top: 6, right: 10, left: -6, bottom: 0 }}>
                <CartesianGrid stroke="rgba(255,255,255,0.05)" vertical={false} />
                <XAxis dataKey="name" tick={{ fontSize: 10, fill: 'rgba(148,163,184,0.6)' }} axisLine={{ stroke: 'rgba(255,255,255,0.06)' }} tickLine={false} height={28} />
                <YAxis tick={{ fontSize: 10, fill: 'rgba(148,163,184,0.45)' }} axisLine={{ stroke: 'rgba(255,255,255,0.06)' }} tickLine={false} width={28} domain={yDomain} />
        ...
        <p className="mt-3 text-[10px] text-slate-600">* Μέσος όρος από όλες τις καταχωρημένες βαθμολογίες ανά τεστ.</p>
      </div>
    </section>
  );
}
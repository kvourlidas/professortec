// src/components/dashboard/DashboardAttendanceTodaySection.tsx
...
export default function DashboardAttendanceTodaySection({ schoolId }: Props) {
    ...
    const load = async () => {
      ...
      try {
        ...
        const [presentRes, absentRes] = await Promise.all([
          supabase.from('class_attendance').select('id', { count: 'exact', head: true })
